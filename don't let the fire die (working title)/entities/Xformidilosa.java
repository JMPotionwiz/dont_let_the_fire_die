...
import core.main;
import items.XfVenisonItem;

public class Xformidilosa extends LivingEntity implements AlwaysTick, Monster {
    protected double baseSpeed = 0.1;
    private double targetXY[] = null;
    private Entity target = null, primeTarget = null;
    private byte aggressiveness, behavior = 0;
    private long targetID = -1, primeTargetID = -1;
    
    public Xformidilosa(int x, int y, Entity target) {
        super(x,y);
        this.hp = 16;
        this.maxHp = 16;
        this.aggressiveness = (byte)(Math.random() * 11);
        this.primeTarget = target;
        
        main.monsters.add(this);
    }
    protected Xformidilosa(String[] in) {
        super(in);
        int i = 18;
        
        this.behavior = Byte.parseByte(in[i++]);
        this.aggressiveness = Byte.parseByte(in[i++]);
    ...
    public void tick() {
        ...
        if (this.hp <= 0 && this.hurtTime == 1) for (int i = (int)(Math.random() * 3) + 1; i > 0; i--) main.spawnIn.add(new ItemEntity(this.X(), this.Y(), new XfVenisonItem(), 2.5));
    }
    protected void control() {
        ...
        switch (this.behavior) {
            case 0:
                ...
                if (this.idleTime <= 0) {
                    ...
                    if (Math.hypot(main.campfire.X() - this.X(), main.campfire.Y() - this.Y()) > 256 && Math.random() < 0.1) {
                        ...
                        this.targetXY = new double[] {this.pos[0] + (x * 64), this.pos[1] + (y * 64)};
                    }
            ...
            case 3:
                if (this.target == null || this.target.hp()[0] <= 0) {
                    ...
                    this.idleTime = (int)(Math.random() * 9) * 60;
                    break;
                } else if (Math.hypot(this.XY()[0] - this.target.XY()[0], this.XY()[1] - this.target.XY()[1]) <= 8 + this.size() + this.target.size()) {
                    if (this.attackCooldown <= 0) {
                        this.behavior = 4;
                        this.attackWarmup = 30;
                    }
                    break;
            ...
            case 4:
                ...
                if (this.attackWarmup <= 0) {
                    ...
                    this.attackCooldown = (int)(Math.random() * 31);
                    this.behavior = 5;
                }
                break;
            case 5:
        ...
        if (Math.abs(move[0]) + Math.abs(move[1]) > 0) {
            if (++this.moveAnim > 24) this.moveAnim = 1;
    ...
    public void render(Graphics2D g) {
        ...
        StringBuilder i = new StringBuilder("x_formidilosa."); 
        if (this.hp()[0] <= 0) {
            i.append("dead");
        ...
        } else if (this.moveAnim > 0 && this.moveAnim <= 18) {
            i.append("walk_");
            if (this.moveAnim <= 6) {
                i.append(0);
            } else if (this.moveAnim <= 12) {
                i.append(1);
            } else i.append(2);
        } else i.append("idle");
        ...
        g.drawImage(main.art.get(i.toString()), this.XY()[0] - 7, this.XY()[1] - 19, null);
    }
    ...
    protected void whenDamaged(Entity e) {
        if (this.behavior == 0 || this.behavior == 1) {
            if (Math.random() < (double)(this.aggressiveness + main.threatLevel) * 0.1) {
                this.target = e;
                this.behavior = 3;
                this.aggroTime = 1200;
            } else {
        ...
        } else if (e != this.primeTarget && (this.behavior == 3 || this.behavior == 4 || this.behavior == 5)) {
            this.target = e;
            this.aggroTime = 600;
        }
    }
    public void sweepAttack() {
        boolean hit = false;
        for (Entity e : main.ticking) if (Math.hypot(this.XY()[0] - e.XY()[0], this.XY()[1] - e.XY()[1]) <= 12 + this.size() + e.size() && e != this)
        if (e.damage(this.attackDamage(), 8, this) != null) hit = true;
        this.attackAnim();
        if (!hit) main.playSound("sfx_playerswing");
    }
    public void attack() {
        boolean hit = false;
        if (Math.hypot(this.XY()[0] - this.target.XY()[0], this.XY()[1] - this.target.XY()[1]) <= 12 + this.size() + this.target.size() && this.target != this)
        if (this.target.damage(this.attackDamage(), 5, this) != null) hit = true;
        this.attackAnim();
        if (!hit) main.playSound("sfx_playerswing");
    }
    
    public int attackDamage() {return 5;}
